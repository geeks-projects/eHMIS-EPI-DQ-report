...
data <- read.csv("./data/dhis2-112023.csv")


data_clean  <- data |> 
mutate( missing_reports = Expected.reports - Actual.reports, 
         missing_reports = if_else(missing_reports == 0, FALSE, TRUE),
         neg_dropout_rate_dpt1_3 = if_else(dropout.rate >= -10, FALSE, TRUE),
         discrepancy_dpt3_pcv3 = abs(((DPT3 - PCV.3)/DPT3)), 
         discrepancy_dpt3_pcv3 = if_else(discrepancy_dpt3_pcv3 <= 0.1, FALSE, TRUE),
...
plot_ug_map <- function(data, fill, title ){
 ...
 ggplot(data) +
  geom_sf(data = data, aes(fill = {{fill}}, group = District),
          color = "white",
          linetype = 1) +
  geom_sf_text(data = data,aes(label = case_when({{fill}} != "0 - 10"~ District, NULL)),size = 2) +
    # Labs
    labs(title = title,
         fill = "%") +
...
#| context: setup
data2 <- vroom("data/dhis2-112023.csv")
...
output$table <- reactable::renderReactable({
   ...
   data2 <- vroom::vroom("data/dhis2-112023.csv")
    ...
    #  dplyr::rowwise() |> 
      ...
      dplyr::mutate(missing_reports = `Expected reports` - `Actual reports`, 
             ...
             neg_dropout_rate_dpt1_3 = dplyr::if_else(`dropout rate` >= -10, FALSE, TRUE),
             discrepancy_dpt3_pcv3 = abs(((DPT3 - `PCV 3`)/DPT3)), 
             discrepancy_dpt3_pcv3 = dplyr::if_else(discrepancy_dpt3_pcv3 <= 0.1, FALSE, TRUE),
